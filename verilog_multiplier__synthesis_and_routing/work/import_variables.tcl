...
set workdir  "/home/vlsi/Desktop/Multiplier1/Lab3_2022" ;        # REQUIRED: Path to project home
...
## REVIEW: Identify the PTF file(s) corresponding to each parasitic condition 
...
set MGC_parasitic_library(new_rc) ${workdir}/lib_data/NCSU_FreePDK_45nm.ptf

## REVIEW: Identify the timing library files (.lib) corresponding to each PVT condition 
...
set MGC_timing_library(new_pvt) ${workdir}/lib_data/NangateOpenCellLibrary_typical.lib

## REVIEW: Provide names for the design analysis corners.
...
## REVIEW: Associate the temperature, parasitic condition and PVT condition to to each analysis corner
...
##########################################################################################################################
set MGC_importVerilogNetlist "${workdir}/integrationMult/verilog/demo_integrationMult.syn.v"                     ; # REQUIRED: Path to vg netlist.  Multiple files/wildcards allowed, but must 
                                                      #           match vg files only. NOT REQUIRED FOR mxdb flow.
set MGC_topDesign            "integrationMult"                ; # REQUIRED  : Name of top level module in verilog netlist(s).
...
set MGC_modes                {new_mode}                     ; # REVIEW: User generated name(s) for design modes. Example: {func shift}

## REVIEW: Associate the design modes (from MGC_modes) to analysis corners (from MGC_corners)
##
## Example:
...
## REVIEW: Identify the timing constraints file(s) (.sdc) for each mode.
##
## Example:
...
set MGC_importConstraintsFile(new_mode) ${workdir}/constraints/demo_adder_func.sdc

set MGC_primary_power_net        ""         ; # REVIEW: Primary power net name.
set MGC_primary_ground_net       ""         ; # REVIEW: Primary ground net name.
set MGC_MultiVoltage             false      ; # ADVANCED: Enable if the design is multivoltage.  UPF file necessary if true.
set MGC_UPF_File                 "${workdir}/constraints/demo_adder.85.upf"         ; # REQUIRED: Unified Power Format (UPF) for defining design power intent.
